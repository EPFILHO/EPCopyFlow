# core/zmq_router.py
# Versão 1.0.9.n
# Author: [Seu Nome ou EPFilho, conforme preferir]
# Description: ZeroMQ Router for MQL5-Python Integration. Receives messages from MQL5 EA and forwards to message handler.
# Updated: 2025-06-12 for Version 1.0.9.n to enhance logging for OHLC_INDICATOR_UPDATE streaming messages and ensure compatibility with EA streaming.
...
class ZmqRouter:
    def __init__(self, broker_manager):
        ...
        self.trade_sockets = {}  # TradePort (ZMQ_DEALER)
        self.stream_sockets = {}  # StrPort (ZMQ_SUB)
    ...
    async def send_command_to_broker(self, broker_key: str, command: str, payload: dict = None, request_id: str = None,
                                     use_data_port: bool = False,
                                     use_trade_port: bool = False):
        """
    ...
    async def _setup_single_broker_sockets(self, broker_key: str, config: dict):
        ...
        # TradePort (DEALER - para comandos de trade e suas respostas)
                    ...
                    for broker_key, socket in socket_type_map.items():
                        if socket in socks and socks[socket] == zmq.POLLIN:
                            try:
                                ...
                                logger.debug(f"Bloco 6 - ZMQ RX de {broker_key} ({port_name}): {message_str[:100]}...")
                                ...
                                try:
                                    ...
                                    # Log adicional para depuração de OHLC_INDICATOR_UPDATE
                                    msg_type = message_data.get("type")
                                    event = message_data.get("event")
                                    if msg_type == "STREAM" and event == "OHLC_INDICATOR_UPDATE":
                                        logger.info(
                                            f"Bloco 6 - Recebida mensagem OHLC_INDICATOR_UPDATE de {broker_key} ({port_name}): request_id={message_data.get('request_id')}")
