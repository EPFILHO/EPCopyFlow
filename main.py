...
# - CORREÇÃO: Centralizada a leitura do config.ini e a configuração do logging,
#   garantindo que o arquivo seja lido apenas uma vez na inicialização e a instância
#   seja compartilhada entre os componentes.
...
class SplashScreen:
    def __init__(self, duration=1.0):
        ...
        version_label = QLabel("Versão 1.0.9.o") # <--- ATUALIZADO PARA A NOVA VERSÃO
...
def setup_logging(config_manager_instance: ConfigManager): # <--- ALTERADO: Recebe a instância de ConfigManager
    ...
    # Carrega o nível de log da instância de ConfigManager
...
async def main_application_flow(config: ConfigManager): # <--- ALTERADO: Agora aceita a instância de ConfigManager
    ...
    # A instância de ConfigManager agora é passada como parâmetro, não criada aqui
...
if __name__ == "__main__":
    # AQUI: A ÚNICA INSTÂNCIA DE CONFIGMANAGER É CRIADA E REUTILIZADA EM TODA A APLICAÇÃO
    initial_app_config = ConfigManager() # <--- CRIADA AQUI

    # Configurar logging o mais cedo possível para que todas as mensagens sejam capturadas
    setup_logging(initial_app_config) # <--- ALTERADO: Passa a instância criada
    ...
    try:
        # Carrega a configuração para a splash screen a partir da instância principal
        ...
        if show_splash_screen:
            ...
            def start_main_app_flow_after_splash():
                try:
                    # Passa a instância principal para main_application_flow
                    qasync.run(main_application_flow(initial_app_config)) # <--- ALTERADO: Passa a instância principal
                except Exception as e:
